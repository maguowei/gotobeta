...
- `server.cors_allowed_origins`（默认 `[]`）是 CORS 跨域来源白名单，逐项精确匹配 `Origin` 并回显；`"*"` 放行任意来源但不携带凭证；空列表表示不放行跨域。
- `server.tls.enabled=true` 时进程内直接终止 TLS（HTTPS），必须同时提供 `server.tls.cert_file` 与 `server.tls.key_file`。
- `server.behind_tls_proxy` 声明部署在 ingress/LB 等 TLS 终止代理之后；prod 环境必须满足 `server.tls.enabled` 或 `server.behind_tls_proxy` 之一，否则校验失败。
